...
import com.moviemux.board.usecase.RateBoardMovieUseCase;
...
public class RateBoardMovieUseCaseImpl implements RateBoardMovieUseCase {
	...
	@Override
	public BoardMovieRating rate(UUID boardPublicId, UUID movieId, UUID requestingUserId, Integer rating)
		...
		BoardMember member = boardMemberRepository
				...
				.orElseThrow(() -> new BoardMemberNotFoundException("You must be an accepted member to rate movies"));
		...
		BoardMovieRating existingRating = boardMovieRatingRepository
				.findByBoardMovieAndUser(boardMovie, member.getUser()).orElse(null);

		if (existingRating != null) {
			existingRating.setRating(rating);
			return boardMovieRatingRepository.save(existingRating);
		}

		BoardMovieRating newRating = BoardMovieRating.builder()
				.boardMovie(boardMovie)
				.user(member.getUser())
				.rating(rating)
				.build();

		return boardMovieRatingRepository.save(newRating);
	}
}
